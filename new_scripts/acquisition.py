...
class Acquisition(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.spectrum_rebin = False # this affects only the spectrum plot
    ...
    def connect_to_tdc(self):
        ...
        for i in range(0,4):
            ...
            self.tagger.set_channel_level(i,-0.5 )
            self.tagger.set_channel_falling(i)
            self.tagger.set_channel_window(i,start = int(self.tdc_start.value()*4000),stop = self.tdc_stop.value()*4000) # 1000*4000 is 1 ms after trig
            
        self.tagger.set_trigger_level(0.5)
    ...
    def read_tag(self):
        self.tag = caget('Scanner:PV')
        print("Tagger", self.tag)
        return self.tag  
    ...
    def calc_rates(self):
        ...
        if len(self.list_data) > 0:
            ...
            if gate1:
                data = data[data[:,5]>gate1[0]*4000]
                data = data[data[:,5]<gate1[1]*4000]
            else:
                data = data
